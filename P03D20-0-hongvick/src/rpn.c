...
int isEmpty(StrStack* root) { return !root; }
...
// according to the shunting yard algorithm,https://en.wikipedia.org/wiki/Shunting-yard_algorithm,accessed on
// 6/11/2019
StrStack* infix_to_postfix(char* arr, char choice, char* variable, char* result) {
    ...
    while (z < strlen(arr)) {
        ...
        } else if (arr[z] == '(' && arr[z + 1] == '-' && is_digit(arr[z + 2])) {
            char* zero = "0";
            char* left_bracket = "(";
            char* minus = "-";
        ...
        // check for operators
        else if (!is_digit(arr[z]) && is_bracket(arr[z]) == 0) {
            if (is_sqrt(arr, z)) {
...
int isnumEmpty(NumStack* root) { return !root; }
...
// learnt how to implement reverse polish notation at https://www.youtube.com/watch?v=7ha78yWRDlE By
// Computerfile Uploaded on 21 Mar 2014 Accessed on 6/11/2019
